...
def train():
        ...
        with torch.no_grad():
            ...
            for itr_obj in range(args.n_object): 
                ...
                if args.render_video:
                    ...
                    try:
                        imageio.mimwrite(filename, rgbs, fps=25, quality=8, format='FFMPEG')
                    except Exception as e:
                        print(f"Video saving failed with error: {e}")
                        print("Trying fallback without quality arg...")
                        try:
                            imageio.mimwrite(filename, rgbs, fps=25, format='FFMPEG')
    ...
    for i in trange(start, N_iters):
        ...
        for itr_obj in range(args.n_object):
            ...
            sigma_torso = F.relu(torch.stack(sigma_torso, dim=0))
            if args.concate_bg:
                # Use out-of-place addition to avoid in-place modification error
                sigma = sigma.clone()
                sigma[-1, :, :, -1] = sigma[-1, :, :, -1] + 1e-6
                sigma_torso = sigma_torso.clone()
                sigma_torso[-1, :, :, -1] = sigma_torso[-1, :, :, -1] + 1e-6
            feat = torch.stack(feat, dim=0)
            feat_torso = torch.stack(feat_torso, dim=0)
        ...
        if (i % args.i_test_person == 0 and i > 0) or (i in [100, 500, 1000, 3000]):
            with torch.no_grad():
                ...
                for itr_obj in range(args.n_object):
                    ...
                    for testimg_i in range(0, len(i_val), 1):
                        img_i = i_val[testimg_i]
                        
                        # Debug: Print audio and expression stats for the first few frames
                        if testimg_i < 5:
                            print(f"DEBUG Frame {testimg_i}: img_i={img_i}")
                            # Access raw audio/exp from dataset
                            raw_aud = dataset[itr_obj]['auds'][img_i]
                            raw_exp = dataset[itr_obj]['exp'][img_i]
                            print(f"  Raw Aud Mean: {raw_aud.mean().item():.4f}, Std: {raw_aud.std().item():.4f}")
                            print(f"  Raw Exp Mean: {raw_exp.mean().item():.4f}, Std: {raw_exp.std().item():.4f}")
