...
def load_audface_data_split(basedir, testskip=1, test_file=None, aud_file=None, exp_file='face.pt',
    ...
    if test_file:
        ...
        for i, frame in enumerate(meta['frames'][::testskip]):
            poses.append(np.array(frame['transform_matrix']))
            # Use loop index 'i' for audio if we are testing with new audio (implied by test_file)
            # But we must be careful. If test_file is transforms_train.json, img_id matches training audio.
            # If we are dubbing, we want to play the new audio sequentially.
            # The current logic uses frame['img_id'] which is tied to the original video frame.
            # If we want to sync with new audio, we should use the sequential index 'i'.
            # However, we need to know if we are in "dubbing mode".
            # Usually, if aud_file is provided and different from training, we might want sequential.
            # Let's assume if the audio length is significantly different from the max img_id, we use sequential index.
            
            # Heuristic: If aud_features is short (e.g. < 10000) and we are using transforms_train (which has img_id up to 10000),
            # we probably want to use sequential indexing for the audio.
            if aud_features.shape[0] < 5000 and frame['img_id'] > aud_features.shape[0]:
                 aud_idx = i
            else:
                 aud_idx = frame['img_id']
            
            auds.append(
                aud_features[min(aud_idx, aud_features.shape[0]-1)]
                )
            exps.append(
